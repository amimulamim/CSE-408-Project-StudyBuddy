...
from app.chat.utils.geminiFormatter import fetch_image_bytes
...
def create_or_continue_chat(
    ...
    # Format user message (bytes, not URLs)
    user_parts = []
    if text:
        user_parts.append(text)
    for f in user_msg.files:
        mime = "application/pdf" if f.file_url.endswith(".pdf") else "image/png"
        data = fetch_image_bytes(f.file_url)
        from google.genai import types
        user_parts.append(types.Part.from_bytes(data=data, mime_type=mime))

    context.append({"role": "user", "parts": user_parts})

    # Get Gemini response
    llm = get_chat_llm("gemini")
    ai_response = llm.send_message(context)
