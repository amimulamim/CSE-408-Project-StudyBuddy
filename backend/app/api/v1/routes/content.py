...
async def generate_content(
    ...
    try:
        ...
        result = await content_generator.generate_and_store_content(
        ...
        return {
            ...
            "metadata": {
                ...
                "topic": request.contentTopic,
                "createdAt": db.query(ContentItem.created_at).filter(ContentItem.id == content_id).scalar() or None
            }
        }
...
async def get_user_content(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching user content")
...
@router.get("/raw/user")
async def get_user_raw_content(
    user: Dict[str, Any] = Depends(get_current_user),
    db: Session = Depends(get_db)
) -> Dict[str, Any]:
    """Lists all raw content generated by the user."""
    try:
        contents = db.query(ContentItem).filter(ContentItem.user_id == user["uid"]).all()
        return {
            "contents": [
        ...
        logger.error(f"Error fetching raw user content for {user['uid']}: {str(e)}")
...
async def update_content(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Error updating content {contentId}: {str(e)}")
...
async def delete_content(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Error deleting content {contentId}: {str(e)}")
